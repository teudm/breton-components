...
export interface Props {
  ...
  interval?: number;
  infinite?: boolean;
  itemsToShow?: number; // Número de itens visíveis
}

const onLoad = ({ rootId, scroll, interval, infinite, itemsToShow = 1 }: Props) => {
  function init() {
    const THRESHOLD = 0.6;
    ...
    const onClickPrev = () => {
      ...
      goToItem(
        isShowingFirst ? items.length - itemsPerPage : (pageIndex - 1) * itemsPerPage,
      );
    };
    ...
    const getElementsInsideContainer = () => {
      ...
      return indices.slice(0, itemsToShow); // Limitar ao número de itens visíveis
    };
      ...
      (elements) =>
        elements.forEach((e) => {
          ...
          if (e.isIntersecting) {
            ...
            e.target.classList.remove("active-slider");
          }
          if (!infinite) {
            if (index === 0) {
              if (e.isIntersecting) {
